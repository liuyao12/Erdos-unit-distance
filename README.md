...
The Moser ring `Z[ω_1,ω_3]`, with `ω_3=(5+√-11)/6=(η+2)/3`, now has a separate 3-adic denominator screen. Its `D` slider keeps sampled points whose coordinates in the integral basis have denominators dividing a power up to `3^D` (after cancellation), while the secondary `W₀` slider controls the Archimedean seed window used before applying the denominator layers.
...
- In the Moser ring view, use `D` as a 3-adic denominator cutoff and `W₀` as the seed cut-and-project radius.
